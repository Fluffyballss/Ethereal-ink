    ...
    <header className="border-b border-black/5 bg-white/80 backdrop-blur-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        <div className="flex items-center gap-3 group cursor-default">
          ...
          <h1 className="font-cinzel font-black tracking-[0.4em] text-2xl">ETHEREAL INK</h1>
        </div>
...
const DiceAnimation: React.FC<{ result: number, success: boolean }> = ({ result, success }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-black/90 flex flex-col items-center justify-center animate-in fade-in duration-300">
       <div className={`w-48 h-48 bg-white border-4 rounded-2xl flex items-center justify-center transition-all duration-500 transform shadow-[0_0_80px_rgba(255,255,255,0.2)] ${isFinal ? (success ? 'border-green-500 scale-110' : 'border-red-500 scale-110') : 'border-black animate-bounce'}`}>
         <span className={`font-cinzel text-8xl font-bold ${isFinal ? (success ? 'text-green-600' : 'text-red-600') : 'text-black'}`}>{displayValue}</span>
       </div>
       {isFinal && (
         <div className="mt-12 text-white font-cinzel text-4xl tracking-[0.5em] uppercase animate-in slide-in-from-bottom-4">
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in duration-500">
      <div className="bg-white p-1 rounded-sm shadow-2xl animate-in zoom-in-95 duration-500" style={{ backgroundColor: rColor, boxShadow: `0 0 100px ${rColor}44` }}>
        <div className="bg-white p-10 max-w-sm text-center rounded-sm">
          <Trophy className="w-16 h-16 mx-auto mb-6 animate-float" style={{ color: rColor }} />
          <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30 mb-2">새로운 보물 획득</p>
          <h3 className="font-cinzel text-3xl font-bold mb-4 tracking-tighter">{loot.name}</h3>
...
const App: React.FC = () => {
  ...
  const renderClassSelect = () => (
    <div className="max-w-6xl mx-auto py-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <p className="text-center text-black/40 mb-16 uppercase tracking-[0.3em] text-xs font-bold">당신을 부르는 고대의 원형을 선택하십시오</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {CLASSES.map(cls => (
          <button
            key={cls.id}
            onClick={() => { playSound('click'); setGameState(p => ({ ...p, characterClass: cls })); setPhase(GamePhase.PET_SELECT); }}
            className="group relative flex flex-col items-center p-10 border border-black/5 hover:border-black transition-all duration-500 hover:shadow-2xl bg-white active:scale-95 rounded-sm overflow-hidden"
          >
            ...
            <h3 className="font-cinzel text-2xl mb-4 tracking-widest group-hover:text-gold-bg transition-colors">{cls.name}</h3>
  ...
  const renderPetSelect = () => (
    <div className="max-w-6xl mx-auto py-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <p className="text-center text-black/40 mb-16 uppercase tracking-[0.3em] text-xs font-bold">당신의 여정을 함께할 영물을 선택하십시오</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {PETS.map(pet => (
          ...
          >
            ...
            <div className="mt-auto w-full pt-4 border-t border-black/5 text-[10px] font-black tracking-widest uppercase text-gold-bg text-center">{pet.effectDescription}</div>
          </button>
        ))}
      </div>
    </div>
  ...
  const renderItemSelect = () => (
    <div className="max-w-6xl mx-auto py-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <p className="text-center text-black/40 mb-16 uppercase tracking-[0.3em] text-xs font-bold">미지의 세계로 가져갈 세 가지 유물을 선택하십시오</p>
  ...
  const renderWorldMap = () => (
    <div className="max-w-7xl mx-auto py-12 animate-in fade-in duration-1000">
      <h2 className="font-cinzel text-5xl mb-4 text-center tracking-widest">세계 지도</h2>
      <p className="text-center text-black/40 mb-16 uppercase tracking-[0.3em] text-xs font-bold">여정의 시작점을 선택하십시오</p>
      <div className="relative aspect-[21/9] bg-white border-2 border-black/10 shadow-2xl overflow-hidden rounded-sm p-4 group">
        ...
        {LOCATIONS.map(loc => (
          <div key={loc.id} className="absolute -translate-x-1/2 -translate-y-1/2 flex items-center group/marker z-10" style={{ left: `${loc.x}%`, top: `${loc.y}%` }}>
            ...
            <div className="w-64 bg-white/95 backdrop-blur-md border border-black/10 p-4 shadow-2xl opacity-0 group-hover/marker:opacity-100 transition-all pointer-events-none -translate-x-2 group-hover/marker:translate-x-4 border-l-4 border-l-gold-bg">
              <div className="flex items-center gap-3 mb-2">
                <span className="text-2xl">{loc.icon}</span>
                <h3 className="font-cinzel text-base font-bold tracking-widest">{loc.name}</h3>
              </div>
              <p className="text-[11px] text-black/60 italic leading-relaxed">{loc.description}</p>
            </div>
          </div>
        ))}
        {loading && (
          <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex flex-col items-center justify-center z-50">
            ...
            <p className="font-cinzel text-xl tracking-widest uppercase">운명의 서사시를 여는 중...</p>
          </div>
        )}
      </div>
    </div>
  ...
  const renderAdventure = () => (
    <div className="max-w-7xl mx-auto py-8 h-[calc(100vh-140px)] flex flex-col lg:flex-row gap-8 overflow-hidden animate-in fade-in duration-700">
      {/* Narrative Panel */}
      <div className="flex-1 bg-white border border-black/5 shadow-2xl rounded-sm flex flex-col overflow-hidden relative">
        <div className="h-1 w-full bg-gradient-to-r from-transparent via-gold-bg to-transparent"></div>
        <div ref={scrollRef} className="flex-1 p-10 overflow-y-auto space-y-8 scroll-smooth">
          {gameState.history.map((log, i) => (
            <div key={i} className="animate-in fade-in slide-in-from-bottom-2 duration-500">
              {log.type === 'choice' ? (
                <div className="flex justify-end my-4"><div className="bg-black text-white px-6 py-2 rounded-full font-cinzel text-xs tracking-widest border border-gold-bg/30">{log.text}</div></div>
              ) : (
                <p className={`text-lg leading-relaxed font-serif ${log.success === false ? 'opacity-40 line-through' : 'opacity-90'}`}>{log.text}</p>
              )}
            </div>
          ))}
          {loading && <div className="flex items-center gap-3 text-black/20 italic text-xs uppercase tracking-widest"><Loader2 className="w-3 h-3 animate-spin" /> 서사시를 기록하는 중...</div>}
        </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {currentScenario?.actions.map((act, i) => (
              <button
                ...
                className="group flex justify-between items-center p-5 border border-black/10 bg-white hover:border-black transition-all active:scale-[0.99] rounded-sm shadow-sm hover:shadow-md"
              >
                <div className="flex items-center gap-6">
                  <span className="text-[10px] font-black opacity-10 group-hover:opacity-100 transition-opacity uppercase tracking-widest">#0{i+1}</span>
                  <span className="font-cinzel text-base font-bold tracking-widest">{act.label}</span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-[9px] font-black uppercase tracking-widest opacity-20 group-hover:opacity-100 transition-opacity">{act.statRequired} DC {act.difficulty}</span>
      ...
      <div className="w-full lg:w-96 flex flex-col gap-6">
        <div className="p-8 bg-white border border-black/5 shadow-xl rounded-sm space-y-8">
          <div className="flex items-center gap-6 group">
            <div className="text-6xl animate-float transform group-hover:-rotate-12 transition-transform">{gameState.characterClass?.icon}</div>
            <div>
              <h3 className="font-cinzel text-2xl font-bold tracking-widest">{gameState.characterClass?.name}</h3>
              <p className="text-[10px] font-black uppercase tracking-widest opacity-30">방랑자 Level 1</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2">
            {['strength', 'intelligence', 'dexterity'].map((s: any) => {
              ...
              return (
                <div key={s} className="bg-black/5 p-3 text-center rounded-sm group hover:bg-gold-bg/5 transition-colors">
                  <p className="text-[8px] font-black uppercase opacity-40 mb-1">{s === 'strength' ? '힘' : s === 'intelligence' ? '지능' : '민첩'}</p>
                  <p className="font-cinzel text-xl font-bold">{val}</p>
                  <p className="text-[9px] font-bold text-gold-bg">({bonus >= 0 ? '+' : ''}{bonus})</p>
                </div>
              );
            })}
          </div>
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="h-2 bg-black/5 rounded-full overflow-hidden border border-black/5 p-0.5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => { playSound('click'); setShowInventory(true); }} className="flex flex-col items-center gap-3 p-6 bg-white border border-black/5 hover:border-black transition-all shadow-lg rounded-sm active:scale-95 group">
            <Package className="w-6 h-6 group-hover:scale-110 transition-transform" />
            <span className="text-[10px] font-black uppercase tracking-widest">인벤토리</span>
          </button>
          <button onClick={() => { playSound('click'); setShowShop(true); }} className="flex flex-col items-center gap-3 p-6 bg-white border border-black/5 hover:border-black transition-all shadow-lg rounded-sm active:scale-95 group">
            <Store className="w-6 h-6 group-hover:scale-110 transition-transform" />
            <span className="text-[10px] font-black uppercase tracking-widest">상점</span>
          </button>
        </div>
      </div>
    </div>
  ...
  const renderInventoryOverlay = () => (
    <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-md flex items-center justify-center p-8 animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-5xl h-[85vh] flex flex-col rounded-sm overflow-hidden shadow-2xl animate-in zoom-in-95">
        <div className="p-8 border-b border-black/5 flex justify-between items-center bg-[#fafafa]">
          ...
          <button onClick={() => setShowInventory(false)} className="text-[10px] font-black uppercase tracking-widest opacity-30 hover:opacity-100 transition-opacity">닫기 (ESC)</button>
        </div>
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          {/* Left: Equipped Slots */}
          <div className="w-full md:w-1/3 p-10 bg-black/[0.02] border-r border-black/5 space-y-8 overflow-y-auto">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30 border-b pb-2">장착 중</h4>
            {['무기', '방어구', '장신구'].map((type: any) => {
              const item = (gameState.equipped as any)[type];
              return (
                <div key={type} onClick={() => item && toggleEquip(item)} className={`p-5 border flex items-center justify-between rounded-sm transition-all shadow-sm group cursor-pointer ${item ? 'bg-white border-black/10 hover:border-black/30' : 'bg-black/[0.01] border-black/5 border-dashed'}`}>
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 border border-black/5 flex items-center justify-center bg-white rounded-sm shadow-sm">
                    ...
                    <div>
                      <p className="text-[8px] font-bold opacity-30 uppercase">{type}</p>
                      <p className={`font-cinzel text-sm font-bold tracking-widest ${item ? 'text-black' : 'text-black/20'}`}>{item?.name || '비어 있음'}</p>
                    </div>
                  </div>
                  {item && <span className="text-[8px] font-black uppercase text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">해제</span>}
                </div>
              );
            })}
          </div>
          ...
          <div className="flex-1 p-10 overflow-y-auto bg-white">
            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] opacity-30 border-b pb-2 mb-6">가방 목록</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {gameState.inventory.length === 0 && <p className="text-xs text-black/20 italic text-center py-10">가방이 비어 있습니다.</p>}
              {gameState.inventory.map(item => {
                ...
                return (
                  <div 
                    ...
                    className={`p-4 border transition-all rounded-sm cursor-pointer hover:shadow-lg relative group ${isEquipped ? 'bg-black/5 border-black shadow-md' : 'bg-white border-black/5'}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-[8px] font-black uppercase px-2 py-0.5 text-white rounded-sm" style={{ backgroundColor: rColor }}>{item.rarity}</span>
                    ...
                    <p className="text-[10px] opacity-50 italic mb-3">"{item.description}"</p>
                    <div className="flex gap-2 text-[8px] font-bold uppercase text-black/40">
                    ...
                    <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity text-[8px] font-black uppercase text-gold-bg flex items-center gap-1">
  ...
  const renderShopOverlay = () => (
    <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-md flex items-center justify-center p-8 animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-4xl h-[70vh] flex flex-col rounded-sm overflow-hidden shadow-2xl animate-in zoom-in-95">
        <div className="p-8 border-b border-black/5 flex justify-between items-center bg-[#fafafa]">
          <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2 text-gold-bg">
              ...
              <span className="font-cinzel text-xl font-bold">{Math.floor(gameState.gold)}</span>
            </div>
            <button onClick={() => setShowShop(false)} className="text-[10px] font-black uppercase tracking-widest opacity-30 hover:opacity-100 transition-opacity">닫기 (ESC)</button>
          </div>
        </div>
        <div className="flex-1 p-10 overflow-y-auto bg-white">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {SHOP_ITEMS.map(item => {
              ...
              return (
                <div key={item.id} className="p-6 border border-black/5 rounded-sm hover:border-black/20 transition-all flex flex-col justify-between group">
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      <span className="text-[8px] font-black uppercase px-2 py-0.5 text-white rounded-sm" style={{ backgroundColor: rColor }}>{item.rarity}</span>
                      <span className="text-[8px] font-black uppercase tracking-widest opacity-40">{item.type}</span>
                    </div>
                    <h5 className="font-cinzel text-lg font-bold tracking-widest mb-2 group-hover:text-gold-bg transition-colors">{item.name}</h5>
                    <p className="text-xs opacity-50 italic mb-4">"{item.description}"</p>
                    <div className="flex gap-4 text-[9px] font-bold uppercase mb-6">
                      {item.statBonuses.strength && <span className="text-red-600">STR +{Math.floor(item.statBonuses.strength)}</span>}
                      {item.statBonuses.intelligence && <span className="text-blue-600">INT +{Math.floor(item.statBonuses.intelligence)}</span>}
                      {item.statBonuses.dexterity && <span className="text-green-600">DEX +{Math.floor(item.statBonuses.dexterity)}</span>}
                    </div>
                  ...
                  <button 
                    ...
                    disabled={!canAfford}
                    className={`w-full py-3 flex items-center justify-center gap-3 font-cinzel text-xs tracking-widest transition-all rounded-sm ${canAfford ? 'bg-black text-white hover:bg-gold-bg' : 'bg-black/5 text-black/20 cursor-not-allowed'}`}
                  >
  ...
  return (
    <div className="min-h-screen bg-[#f8f6f2] pb-20 overflow-x-hidden selection:bg-black selection:text-white">
      ...
      {phase >= GamePhase.WORLD_MAP && (
        <div className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-md border-t border-black/5 px-8 py-3 flex justify-between items-center z-[100] text-[9px] tracking-[0.2em] font-black uppercase shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
          <div className="flex gap-12">
            <div className="flex items-center gap-3"><span className="opacity-30">직업</span><span className="font-cinzel text-xs text-gold-bg tracking-widest">{gameState.characterClass?.name}</span></div>
            <div className="flex items-center gap-3 group"><Coins className="w-3 h-3 text-gold-bg" /><span className="font-cinzel text-xs text-gold-bg tracking-widest">{Math.floor(gameState.gold)} GOLD</span></div>
          </div>
          <div className="flex gap-12 items-center">
            <div className="flex items-center gap-3"><Heart className="w-3 h-3 text-red-500 fill-current animate-pulse" /><span className="font-cinzel text-xs">{Math.floor(gameState.hp)} / {Math.floor(gameState.maxHp)}</span></div>
            <div className="flex items-center gap-3"><Zap className="w-3 h-3 text-blue-500" /><span className="font-cinzel text-xs">{gameState.karma > 0 ? '+' : ''}{Math.floor(gameState.karma)}</span></div>
          </div>
        </div>
      )}
    </div>
